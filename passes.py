def passes(pass_df): 
    column_suffixes = ['player', 'team', 'pos_x', 'pos_y', 'action', 'start_time', 'end_time']
    shift_column_names = []
    ...
    for index, row in pass_df.iterrows():
        ...
        while y < upper_shift_limit:
            if y != 0:
                if row[str(y) + '_player'] == row['player'] and row[str(y) + '_start_time'] == row['start_time'] and row[str(y) + '_end_time'] == row['end_time'] and row[str(y) + '_action'] == 'Passes (inaccurate)':
    ...
    for index, row in pass_df.iterrows():
        ...
        while y < upper_shift_limit:
            if y != 0:
                if row[str(y) + '_player'] == row['player'] and 'Cross' in row[str(y) + '_action'] and row[str(y) + '_start_time'] == row['start_time'] and row[str(y) + '_end_time'] == row['end_time']:
                    cross_bool = 1
                    break
                if row[str(y) + '_player'] != row['player'] and row['action'] == 'Crosses (inaccurate)' and row[str(y) + '_start_time'] == row['start_time'] and row[str(y) + '_end_time'] == row['end_time']: 
                    found_next_name = True
                    receiver.append(row[str(y) + '_player'])
                    successful.append(0)
            ...
            if row['action'] != 'Crosses (inaccurate)':
                if row['action'] == 'Assists':
                    if row[str(x) + '_action'] == 'Goals': 
                        found_next_name = True
                        receiver.append(row[str(x) + '_player'])
                        ...
                        break
                elif row[str(x) + '_player'] != row['player']:
